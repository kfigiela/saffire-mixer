...
module SaffireLE.Mixer.Stereo.LowRes where
...
import           SaffireLE.Mixer.Raw.LowRes (MatrixMixer (..), Mix (..))
...
toStereoMixer mix =
    StereoMixer
    { _stereo1 = lowResMixToStereo (mix ^. L.out1, mix ^. L.out2)
    , _stereo2 = lowResMixToStereo (mix ^. L.out3, mix ^. L.out4)
    , _out12ToSpdif = mix ^. L.out12ToSpdif
    }

fromStereoMixer :: StereoMixer -> MatrixMixer
fromStereoMixer mix =
    MatrixMixer
    { L._out1 = out1
    , L._out2 = out2
    , L._out3 = out3
    , L._out4 = out4
    , L._out12ToSpdif = mix ^. out12ToSpdif
    }
    where
